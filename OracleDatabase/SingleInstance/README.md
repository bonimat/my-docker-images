...
##### Note personali
MB: la cartella oradata deve avere onwer **matteo** permessi **777**.  
Usato:
```
docker run --name oraclaxetest -p 11521:1521 -p 15500:5500 -e ORACLE_PWD=bonimat -e ORACLE_CHARACTERSET=AL32UTF8 -v $ORADATATEST:/opt/oracle/oradata oracle/database:18.4.0-xe
```
Una volta funzionato, per connettersi si puo' usare questo comando:
```
docker exec -it oraclaxetest sqlplus sys/bonimat@localhost:1521/XE as sysdba 
```

Per abilitare Oracle Enterprice Edition (OEM) 
(https://docs.oracle.com/en/cloud/paas/database-dbaas-cloud/csdbi/access-em-database-express-18c.html)
è necessario 
entrare dentro il container 
```
 docker exec -it oraclaxetest /bin/bash
```

e abilitare l'accesso remoto da utente **system**:

```
 sqlplus system/bonimat@localhost:1521/XE
```
```
 EXEC DBMS_XDB.SETLISTENERLOCALACCESS(FALSE);
```
Così facendo ci si potrà connettere al gestionale web (https://localhost:15500/em/login).
Le chiavi si accesso sono:

Nome utente sys  
Password < passwords che si usa per la connessione >  
Nome contenitore < vuoto >  

Se nel "Nome contenitore" si mettesse un nome il sistema non da l'accesso ma richiede delle credenziali che nn si sa quale siano.

La connessione al DB da sqlplus esterno 
```
sqlplus "sys/bonimat@(DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)(HOST=localhost)(PORT=11521))(CONNECT_DATA=(SERVER=DEDICATED)(SERVICE_NAME=XE))) as sysdba" 
```
che pero' forse richiede:
```
 EXEC DBMS_XDB.SETLISTENERLOCALACCESS(FALSE);
``` 
Si puo' usare tnsname.ora messo nella cartella $ORACLE_HOME dell'instantclient:
...
```
Così da poter lanciare il comando più semplice:
```
 sqlplus sys/bonimat@XE as sysdba 
```
